...
#include "CPU6502.h"
    ...
    uint8_t CPU6502::GetFlag(CPU6502::FLAG6502 f) {
...
//Addressing Modes
    ...
    uint8_t CPU6502::IZX() {
        uint16_t relLo = read(pc) + x;
        pc++;
        relLo &= 0x00FF;

        uint16_t lo = read(relLo);
        uint16_t hi = read((relLo + 1) & 0x00FF);
...
//Instructions
    ...
    uint8_t CPU6502::ASL() {
        ...
        if (lookup[opcode].addrmode == &CPU6502::IMP) {
            ...
            write(addr_abs, (uint8_t) (temp & 0x00FF));
        }
        return 0;
    }
    ...
    uint8_t CPU6502::ROL() {
        ...
        SetFlag(C, fetched & 0xFF00);
        SetFlag(N, temp & 0x80);
        SetFlag(Z, a = 0x00);
        if (lookup[opcode].addrmode == &CPU6502::IMP) {
            a = temp & 0x00FF;
        } else {
            write(addr_abs, temp & 0xFF00);
        }
        return 0;
    }
